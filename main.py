...
def enhance_image(image, brightness, contrast, sharpness, saturation, noise_reduction):
    ...
    enhanced_image = cv2.addWeighted(
        enhanced_image, contrast, np.zeros_like(enhanced_image), 0, 0)

    # Perform sharpening using Laplacian operator
    laplacian = cv2.Laplacian(enhanced_image, cv2.CV_64F)
    sharpened_image = cv2.convertScaleAbs(
        enhanced_image - sharpness * laplacian)
    ...
    enhanced_image = cv2.bilateralFilter(
        enhanced_image, 9, noise_reduction, noise_reduction)
...
def process_image(image_path, brightness, contrast, sharpness, saturation, noise_reduction):
    ...
    enhanced_image = enhance_image(
        image, brightness, contrast, sharpness, saturation, noise_reduction)
...
def main():
    ...
    parser.add_argument('--brightness', type=float, default=1.0,
                        help='Brightness value (default: 1.0)')
    parser.add_argument('--contrast', type=float, default=1.0,
                        help='Contrast value (default: 1.0)')
    parser.add_argument('--sharpness', type=float, default=0.5,
                        help='Sharpness value (default: 0.5)')
    parser.add_argument('--saturation', type=float, default=1.0,
                        help='Saturation value (default: 1.0)')
    parser.add_argument('--noise_reduction', type=float,
                        default=10.0, help='Noise reduction value (default: 10.0)')
    ...
    if not os.path.exists(args.directory):
        # Add print statement to notify user
    ...
    if os.path.isfile(args.directory):
        ...
        # Add print statement to notify user
        print(f"Invalid input: {args.directory}")
        return False
